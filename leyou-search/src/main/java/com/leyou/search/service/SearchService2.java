...
//    private GoodsRepository goodsRepository;
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        List<SpecParam> params = specClient.queryParams(null, spu.getCid3(), true, null);
        ...
        //获取通用规格参数
        Map<Long, String> genericSpec = com.leyou.search.utils.JsonUtils.toMap(spuDetail.getGenericSpec(), Long.class, String.class);
        //获取特有规格参数
        ...
        goods.setSpecs(map);
        goods.setSkus(com.leyou.search.utils.JsonUtils.toString(skus));
        return goods;
    }
    ...
    private List<Map<String, Object>> handleSpecs(Long id, QueryBuilder basicQuery) {
        ...
        //查询可过滤的规格参数
        List<SpecParam> params = specClient.queryParams(null, id, true, null);
    ...
    private List<Brand> handleBrandAgg(LongTerms terms) {
        ...
        try {
            //得到桶里面的内容。是long类型的列表。
            //getKeyAsNumber().longVvalue()就得到long类型的数据
    ...
    public void insertOrUpdate(Long id) {
        ...
        if (spu == null) {
            System.out.println("索引对应的spu不存在，spuId:"+id);
...
//            queryBuilder.withSort(SortBuilders.fieldSort(sortBy).order(desc ? SortOrder.DESC : SortOrder.ASC));
//        }
//        System.out.println("获取到的sortBy和desc："+desc+"-"+sortBy);
//
//        // 4.2、查询，获取结果
